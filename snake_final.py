...
class Snake:
    ...
    def ideal_safe(self):
        ideal_pos = tuple(self.ideal())
        neighbors = self.get_available_neighbors(self.head.pos)
        path = []
        if ideal_pos in neighbors:
            path.append(ideal_pos)
            v_snake = self.create_virtual_snake()
            for move in path:
                v_snake.go_to(move)
                v_snake.move()
    ...
    def set_path(self):
        ...
        v_snake = self.create_virtual_snake()
        bfs_path = v_snake.bfs(tuple(v_snake.head.pos), tuple(v_snake.apple.pos))
        ...
        if bfs_path:
            ...
            v_snake.add_square()
            v_path = v_snake.get_path_to_tail()


        if v_path:
            return bfs_path

        if self.ideal_safe() and self.moves_without_eating < MAX_MOVES_WITHOUT_EATING/2:
            return self.ideal_safe()
